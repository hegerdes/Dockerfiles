...
for VARIANT in ${VARIANTS[@]}; do
    ...
    docker buildx build --cache-from type=local,src=/tmp/docker-cache --cache-to=type=local,dest=/tmp/docker-cache,mode=max --platform $PLATFORMS --pull --build-arg VARIANT=$VARIANT --build-arg PANDOC_URL=$PANDOC_DOWNLOAD_URL -f "Dockerfile.base" -t $IMAGE_NAME:${VARIANT_NAME_TAG[0]} $DO_PUSH .
done
